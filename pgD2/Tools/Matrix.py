...
class Matrix(object):
    ...
    def paint_matrix_field(self, center, size, text, length=None, mode = 1):
        ...
        if len(text) > length:
            text = text[0:length] if mode == 0 else (text[:-length]) #1: All to zero(+)(first out), 2: zero to all(-)(last out)
        ...
        if len(text) < length:
                ...
                for i in range(num_blanks):
                    ...
                    mat.paint_matrix(' ', (pos_x, center[1]), size)
